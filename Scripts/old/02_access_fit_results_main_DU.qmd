...
fit_file <- 'fit_n_142_2023-09-25_bandit2arm_delta_main_DU_estimation1_delta0.99_stepsize0.1.rds'
...
if (estimation==1) {
  write.table(params, file=file.path(out_path, 'Params_main_DU.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = c("ID","aud_group","reinforcer_type","block","p_corr","alpha","invtemp"))
}
```
...
if (estimation==1) {
  write.table(mods, file=file.path(out_path, 'fmri_param_modulators_main_DU.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
}
```
