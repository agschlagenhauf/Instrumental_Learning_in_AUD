---
title: "Prior Predictive Checks Hierarchical Main Model Results"
...
fit_file <- 'fit_n_71_2024-01-03_bandit2arm_delta_main_hierarchical_estimation0_delta0.9_stepsize0.5.rds'
...
if (estimation==1) {
  ...
  write.table(log_likelihood, file=file.path(out_path, 'log_lik_main_hierarchical.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
...
a <- c(1:(stan_data$N*44)) # vector with number from 1 to 71 subjects * 44 trials
b <- c(stan_data$N*50+1:(stan_data$N*94-stan_data$N*50)) # vector with numbers from 71 subjects * 50 trials + 1 to 71 subjects * 94 trials.
...
if (estimation==1) {
  write.table(params, file=file.path(out_path, 'Params_main_hierarchical.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = c("ID","aud_group","reinforcer_type","block","p_corr","alpha","invtemp"))
}
```
...
if (estimation==1) {
  write.table(PE_summary_df, file=file.path(out_path, 'fmri_PEs_main_hierarchical.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
}
```
...
if (estimation==1) {
  write.table(Q_summary_df, file=file.path(out_path, 'fmri_Qs_main_hierarchical.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
}
```
...
if (estimation==1) {
  write.table(pc_summary_df, file=file.path(out_path, 'fmri_choiceprobs_main_hierarchical.txt'), append = FALSE, sep = " ", dec = ".", row.names = F, col.names = F)
}
```
